...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "dm-process-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name dm-process-service apps/dm-process-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app dm-process-service:1.0-SNAPSHOT
		call oc expose svc/dm-process-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd dm-process-service
		cd target
