...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "dm-process-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name dm-process-service apps/dm-process-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app dm-process-service:1.0-SNAPSHOT
    oc expose svc/dm-process-service
else

	echo "Launching the application locally..."
	pattern="dm-process-service"
	files=( $pattern )
	cd ${files[0]}
